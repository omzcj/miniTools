...
- 合盖运行：手动保持 MacBook 合盖后继续运行；支持 1、2、4、8 小时或不限时会话、可配置低电量保护，并在严重过热、应用或后台服务异常时自动恢复睡眠
...
设置中的低电量保护可选 10%–50%，所有运行时长都保留低电量、严重过热和异常恢复保护。设置中会保留最近 5 次会话的关闭时间、原因与运行方式。该功能会增加耗电与发热，
不应在收纳包内长时间使用。

...
- 合盖运行：手动保持 MacBook 合盖后继续运行；支持时长限制，并在开盖、低电量、严重过热、应用或后台服务异常时自动恢复睡眠
...
状态栏菜单中的“合盖运行”使用勾选状态显示当前开关。第一次开启需要在“系统设置 →
通用 → 登录项与扩展”中批准 miniTools 后台服务；之后不再重复请求管理员批准。最长运行
时间可选择 2、4、8、12 小时或不限时，其中“不限时”仍保留开盖、低电量、严重过热和
异常恢复保护。设置中会保留最近一次会话的关闭时间与原因。该功能会增加耗电与发热，
不应在收纳包内长时间使用。

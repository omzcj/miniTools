...
- 合盖运行：手动保持 MacBook 合盖后继续运行；可配置低电量保护，并在严重过热、应用或后台服务异常时自动恢复睡眠
...
状态栏菜单通过单个“启动合盖运行”或“关闭合盖运行”项目直接切换状态；启动后持续运行，直到手动关闭或安全保护触发。
第一次开启需要在“系统设置 → 通用 → 登录项与扩展”中批准 miniTools 后台服务；之后不再重复请求管理员批准。
设置中的低电量保护可选 10%–50%，运行期间始终保留低电量、严重过热和异常恢复保护。设置中会保留最近 5 次会话的关闭时间、原因与运行方式。该功能会增加耗电与发热，
不应在收纳包内长时间使用。
...
- `Features/ClosedLidRunning`：合盖运行会话、电量/温度保护和特权 helper 客户端

...
- 合盖运行：手动保持 MacBook 合盖后继续运行；支持有限时长或不限时会话、可配置低电量保护，并在开盖、严重过热、应用或后台服务异常时自动恢复睡眠
...
状态栏菜单提供“合盖运行 X 小时”和“合盖运行 不限时”；勾选项表示当前会话模式。
第一次开启需要在“系统设置 → 通用 → 登录项与扩展”中批准 miniTools 后台服务；之后不再重复请求管理员批准。
设置中的默认最长时长可选 2、4、8、12 小时，低电量保护可选 10%–50%；“不限时”仅在每次开启时从状态栏显式选择，仍保留开盖、低电量、严重过热和异常恢复保护。设置中会保留最近一次会话的关闭时间与原因。该功能会增加耗电与发热，
不应在收纳包内长时间使用。

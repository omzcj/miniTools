...
- 状态栏提供“合盖运行 X 小时”和“合盖运行 不限时”两个可勾选项；有限时长来自设置的默认值，勾选状态表示当前会话模式。
- 合盖运行仅允许用户手动开启或关闭，不检测 Codex、Claude 或其他进程状态，也不增加自动触发规则。
- helper 只允许开启、关闭和查询合盖运行状态，不得接受任意命令、路径或 `pmset` 参数。
- 设置中的默认最长时长可选 2、4、8、12 小时，默认 8 小时，不提供“不限时”；不限时会话只能从状态栏显式开启。
- 低电量保护阈值可选 10%、20%、30%、40%、50%，默认 20%，不允许关闭。
- 开启后发生开盖、电池供电且电量低于设定阈值、严重/临界热状态、主应用连接异常或重启时必须恢复系统睡眠。
- helper 异常退出时由 launchd 立即重新拉起并通过恢复哨兵恢复系统睡眠；主应用运行期间每 15 秒核对一次 helper 的真实状态。
- 最近一次会话的开启状态、关闭时间和关闭原因需要持久化，设置界面用于解释自动关闭结果，不能只展示当前开关。

...
- 状态栏的“合盖运行”是二级菜单，顶部为“关闭”，分隔线下按顺序提供不限时、1、2、4、8 小时；关闭时勾选“关闭”，运行时只勾选当前会话时长。
- 合盖运行仅允许用户手动开启或关闭，不检测 Codex、Claude 或其他进程状态，也不增加自动触发规则。
- helper 只允许开启、关闭和查询合盖运行状态，不得接受任意命令、路径或 `pmset` 参数。
- 设置中不提供最长时长配置；每次会话的时长必须从状态栏二级菜单显式选择。
- 低电量保护阈值可选 10%、20%、30%、40%、50%，默认 20%，不允许关闭。
- 开盖不结束当前会话；再次盒盖时继续沿用原运行方式。电池供电且电量低于设定阈值、严重/临界热状态、主应用连接异常或重启时必须恢复系统睡眠。
- helper 异常退出时由 launchd 立即重新拉起并通过恢复哨兵恢复系统睡眠；主应用运行期间每 15 秒核对一次 helper 的真实状态。
- 当前会话和最近 5 次关闭记录需要持久化；关闭记录包含时间、原因和运行方式，设置界面用于解释自动关闭结果。
